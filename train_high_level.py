...
def save_checkpoint(episode, dqn, optimizer, buffer, epsilon, train_rewards_log, train_loss_log):
...
def load_checkpoint(dqn, optimizer):
    if not os.path.exists('checkpoints/training_state.pkl'):
        return None

    # dqn_state = torch.load('checkpoints/dqn_latest.pth')
...
def main():
    ...
    checkpoint = load_checkpoint(dqn, optimizer)
    ...
    for episode in range(start_episode, NUM_EPISODES):
        ...
        save_checkpoint(episode, dqn, optimizer, buffer, epsilon, train_rewards_log, train_loss_log)
